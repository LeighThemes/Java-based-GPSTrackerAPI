...
import gov.nasa.worldwind.util.WWtil; 
...
public class GPSTrackAPI extends ApplicationTemplate{
	...
	public AppFrame() throws IOException {
		...
		this.balloonController = new BalloonControllef (this. getWwd ());
...
for (int i = words.length - 1; i >= 0; 1--) {
...
// This is for the datailed map layer
...
Position balloonPosition = null;
private static Position AppleReader() throws I0Exception {
// To get values from value.txt, just change the location below
	String fileName = "C:\\Users\\5G_Server\|Desktop\\Apple.txt":
	Position balloonPosition = null;
	...
	String one = strs[13];
	String two = strs[9];
	double valuel - Double.parseDouble(one);
	double value2 = Double.parseDouble(two);
	System.out.printIn(one);
	System.out.printIn (two);
	balloonPosition = Position. fronDegrees (value, valuel);
...
Balloon balloon = new
ScreenAnnotationBalloon ("<b›Current GPS Location:</b›" + AppleReader(),
new Point (50, 300));
...
attrs.setOffset (new Offset (Od, Od, AVKey. PIXELS, AVKey. PIXELS)); 
